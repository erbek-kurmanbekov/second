 ...
 const renderToDos = ()=>{
     ...
     tasks.forEach(el =>{
         ...
         trash.addEventListener('click', ()=>{
             let con = confirm('Вы точно хотите удалить запись?')
             if(con){
        ...
        correct.addEventListener('click', ()=>{
            ...
            tasks.forEach(el =>{
                if(el.id == id){
                let p = confirm('Вы действительно хотите заменить запись?')
                if(p){
                    el.name = cor
                }
            }
...
//     'bootcamp': 'ITC'
// }
// stringify - переводит в объект
// JavaScript в строку JSON, возможно с заменой значений,
// если указана функция замены, или с включением только определенных свойств, 
// если указан массив замены.
// let objTranslate = JSON.stringify(obj)
// alert(objTranslate)
// parse - переводит в текст
